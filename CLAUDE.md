...
- **[Phase 4.x] Investigation "Load request cancelled" mobile-safari sur /bri-1.avif** : entrée empirique observée dans l'artefact CI e52da93 (mobile-safari uniquement, 1 occurrence). Test E2E `bri-*.avif sans 404` passe en chromium-desktop, donc l'asset existe et est servi par Vite. Bug runtime mobile-safari spécifique : suspect re-render React qui annule la requête image avant fin de chargement. Hypothèses : (a) useEffect avec deps changeantes dans ChateauCard re-mounté lors du scroll, (b) cleanup async-ready Phase 2.3 qui abort prematurément, (c) timing prefetch Safari throttlé. À reproduire local (Safari macOS ou simulateur iOS) puis diagnostiquer. ~2-3h. Identifié le 7 mai 2026 par analyse de l'artefact CI run e52da93.
