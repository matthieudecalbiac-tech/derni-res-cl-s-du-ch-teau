...
CREATE TABLE IF NOT EXISTS public.chateaux (
  ...
  ordre_une                   integer,                               -- rang section "Les clés à la une" (⚠ null = PAS à la une)
  ordre_home                  integer,                               -- ordre section "Découvrez aussi" (null = affiché, mais fin de tri)
  ...
  -- Opt-in du moteur de disponibilité (migration 2026-08-23). Dormant tant que
  -- estDisponible n'existe pas. false = la table disponibilites est ignorée
...
COMMENT ON COLUMN public.chateaux.dispo_geree IS
  'Opt-in du moteur de disponibilité. false (défaut) = la table disponibilites est IGNORÉE pour ce château, comportement historique (proxy éditorial urgence). true = la table FAIT FOI, et une date SANS LIGNE se tranche par dispo_ouverte_jusqu_a : OUVERTE jusqu''à cet horizon inclus, FERMÉE au-delà. ⚠ BASCULER À true SANS POSER D''HORIZON FERME LE CHÂTEAU ENTIÈREMENT — dispo_ouverte_jusqu_a à NULL vaut « aucune ouverture par défaut », donc aucune nuit réservable. C''est le garde-fou que l''écran d''édition admin annonce depuis le 24 août 2026, en trois versions dérivées de l''état. La bascule se fait château par château, APRÈS avoir posé l''horizon (cf. piège 1 de l''audit disponibilités du 22 août 2026).';
...
COMMENT ON TABLE  public.disponibilites IS
  'Calendrier disponibilités par chambre. Une ligne = 1 chambre × 1 date. ⚠ L''interprétation d''une date SANS LIGNE dépend de DEUX colonnes de chateaux : si dispo_geree est false (défaut), la table n''est pas consultée du tout ; si dispo_geree est true, une date sans ligne est OUVERTE tant qu''elle est <= dispo_ouverte_jusqu_a (borne incluse) et FERMÉE au-delà — c''est l''horizon du 24 août 2026. ⚠ Le châtelain ne saisit donc QUE ses blocages : opt-OUT BORNÉ, nommé comme tel (un oubli de blocage OUVRE une nuit ; l''horizon est la borne qui rend ce risque fini). Ce n''est PAS l''opt-in strict décrit ici du 23 au 24 août, où toute date sans ligne était fermée ; ni l''opt-out d''origine — « Absence = disponible au prix par défaut » — jamais implémenté, et qui aurait tout ouvert SANS BORNE sur une table vide.';
COMMENT ON COLUMN public.disponibilites.est_disponible IS
  'Ouverture EXPLICITE de la date — la seule des trois réponses possibles qui soit ÉCRITE. false = bloquée (entretien, occupation privée, séjour hors plateforme). true = ouverte, et ⚠ PLUS FORTE QUE L''HORIZON : elle ouvre une nuit au-delà de chateaux.dispo_ouverte_jusqu_a (un mariage réservé deux ans à l''avance, sans déplacer l''horizon pour tout le monde). ⚠ Ne pas confondre avec l''ABSENCE de ligne, qui n''est pas une valeur : elle se tranche par l''horizon (cf. le commentaire de la table). ⚠ C''EST POURQUOI « rouvrir » EFFACE la ligne au lieu d''en écrire une à true — une ligne true resterait ouverte au-delà de l''horizon, que plus rien ne refermerait. Une réservation confirmée n''a pas besoin d''être reportée ici : elle est dérivée à la lecture depuis reservations, comme le palier du Club, pour qu''il n''y ait jamais deux représentations du même fait.';
