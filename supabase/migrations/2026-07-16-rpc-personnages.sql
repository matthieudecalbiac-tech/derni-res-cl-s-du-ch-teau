-- ============================================================
-- admin_upsert_chateau — bloc 6 : chateau_personnages (get-or-create + REPLACE)
--
-- SEUL le bloc 6 (personnages) est AJOUTE + le nouveau parametre p_personnages +
-- le COMMENT. Blocs 1-5 (garde is_admin, base 34 colonnes, chambres DIFF,
-- timeline/alentours REPLACE, amenities CTE) BYTE-IDENTIQUES a la version
-- amenity/equipements (2026-07-15-equipements).
--
-- SIGNATURE : on AJOUTE p_personnages jsonb DEFAULT NULL (7e parametre). Comme la
-- signature change, un simple CREATE OR REPLACE creerait une SURCHARGE a cote de
-- la version 6-parametres (appel a 6 args -> ambigu). D'ou le DROP de l'ancienne
-- signature avant le CREATE. Le 7e parametre etant DEFAULT NULL, les appels
-- existants (saveChateauComplet a 6 args) restent valides (p_personnages = NULL).
--
-- Contrat null/[] identique aux autres blocs : null = section preservee,
-- [] = videe, [...] = remplacee.
--
-- Chaque entree du payload : { nom, slug, nature, texte, ordre }. Le slug est
-- calcule EN JS (src/utils/slug.js) et arrive tout fait : la RPC ne slugifie PAS
-- (une seule normalisation, pas de divergence SQL/JS possible).
--
-- Personnage = referentiel PARTAGE -> get-or-create (pas le motif CTE uuid
-- pre-genere du bloc 5, qui suppose des lignes toujours neuves). PIEGE : deux
-- entrees du meme slug feraient echouer ON CONFLICT DO UPDATE (21000, cannot
